...
from module.pubgpy import player, Client, Season
...
class CacheData:
    ...
    def get_lastupdate(
        ...
        cur = self.database.cursor(pymysql.cursors.DictCursor)
        player_id = player_id.id if isinstance(player_id, player.Player) else player_id
        command = pymysql.escape_string("SELECT %s FROM player_data WHERE player_id = %s")
        cur.execute(command, (self._get_last_update(cls), player_id))
        data = cur.fetchone()
    ...
    async def get_playdata(
        ...
        if data is None or (datetime.now() - last_update).days >= 2:
            ...
            if data is None:
                self.save_play_data(player_id=player_id, season=season, data=data)
            else:
