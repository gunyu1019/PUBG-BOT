...
async def response_num(response, ctx):  # 에러 발생시, 코드를 통하여 분석
...
async def season_status(ctx, player_id, season, pubg_platform):
    connect = getDatabase()
    cur = connect.cursor()
    ...
    try:
        ...
        if await player_module.autopost("normal"):
            ...
            if response.status_code == 200:
                ...
                await response_num(response, ctx)
...
async def season_status_update(player_id, season, message, pubg_platform):
    ...
    player_module = p_info.player(player_id)
    try:
        url = "https://api.pubg.com/shards/" + platform_site[pubg_platform] + "/players/" + str(
            player_id) + "/seasons/" + str(season)
        response = await requests("GET", url)
        if response.status_code == 200:
            return_value = response.json()
        else:
...
async def ranked_status(ctx, player_id, season, pubg_platform):
    connect = getDatabase()
    cur = connect.cursor()
    player_module = p_info.player(player_id)
    try:
        ...
        if await player_module.autopost("ranked"):
            url = "https://api.pubg.com/shards/" + platform_site[pubg_platform] + "/players/" + str(
                player_id) + "/seasons/" + str(season) + "/ranked"
            response = await requests("GET", url)
            if response.status_code == 200:
                return_value = response.json()
            else:
