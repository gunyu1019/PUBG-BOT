...
class Player:
    ...
    async def player_platform(
        ...
        embed = discord.Embed(
            ...
            description=comment('platform', 'platform_selection_description', self.language),
            color=color
        )
        components = [
        ...
        try:
            result: interaction.ComponentsContext = await self.client.wait_for_global_component(
                check=lambda x: (
                        x.custom_id in ['steam', 'kakao', 'xbox', 'psn'] and
                        x.component_type == 2 and
                        x.author.id == self.ctx.author.id
                ), timeout=300
            )
        except asyncio.TimeoutError:
            original_description = embed.description
            embed.description = original_description + comment('result', 'timeout_description', self.language)
            for index, _ in enumerate(components[0].components):
                components[0].components[index].disabled = True
            await self.ctx.edit(embed=embed, components=components)
            return

        embed.description = comment('platform', 'player_search_description', self.language)
        for index, _ in enumerate(components[0].components):
            components[0].components[index].disabled = True
        ...
        try:
            player_info = await request_loop(self.ctx, self.pubg_client.player, nickname=nickname)
        except pubgpy.NotFound:
            embed = discord.Embed(
                title=comment('basic', 'error', self.language),
                description=comment('platform', 'player_not_found_description', self.language),
                color=error_color
            )
            await self.ctx.edit(embed=embed)
            return
