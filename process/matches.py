...
class MatchesProcess(ProcessBase):
    ...
    async def load_matches_id(self, session: AsyncSession | None = None):
        if len(self.player.matches) == 0:
            ...
            query = select(database.MatchesPlayer).where(
                database.MatchesPlayer.account_id == self.player.id
            )
            ...
            if len(match_data) == 0:
                embed = discord.Embed(
                    description=comment(
                        "matches_process", "matches_not_found", self.context.locale
                    ),
    ...
    async def update_matches_id(self, session: AsyncSession | None = None):
        ...
        query = delete(database.MatchesPlayer).where(
            database.MatchesPlayer.account_id == self.player.id
        )
    ...
    async def match_info(
        ...
        map_image = MapAssets(
            player_id=self.player.id, map_name=data.map, data=assets_data
        )
        ...
        file = discord.File(
            image, filename="{}_matches_stats.png".format(self.player.name)
        )
