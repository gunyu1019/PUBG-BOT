...
async def autopost1():
    ...
    while not client.is_closed():
        ...
        for i in range(len(client.guilds)):
            total += client.guilds[i].member_count
...
async def autopost3():
    ...
    while not client.is_closed():
        ...
        try:
            cur = connect.cursor()
            sql = f"SELECT last_update, Steam FROM SEASON_STATUS"
            cur.execute(sql)
            cache = cur.fetchone()
            last_update = cache[0]
            html1 = cache[1]
            ...
            if response.status_code == 200:
                ...
                if date_json2 != html2:
                    ...
                    for i in platform_site[1:]:
                        response = await requests.get(f"https://api.pubg.com/shards/{i}/seasons",headers=header)
                    ...
                    sql = 'UPDATE SEASON_STATUS SET Steam=%s,Kakao=%s,XBOX=%s,psn=%s,Stadia=%s,last_update=%s WHERE id=1'
...
async def profile(message,prefix,command):
    ...
    if command == "Information":
        try:
            ...
            if pubg_platform >= 0 and pubg_platform <= 1:
                type_season = "pc-2018"
            else:
                type_season = "console"
            if len(s_count) < 2:
                season = f"division.bro.official.{type_season}-0" + s_count
            else:
                season = f"division.bro.official.{type_season}-" + s_count
        except Exception:
            connect = pymysql.connect(host=db_ip, user=db_user, password=db_pw,db='PUBG_BOT', charset='utf8')
            try:
                cur = connect.cursor()
...
async def on_ready():
    ...
    total = 0
    for i in range(len(client.guilds)):
        answer = answer + str(i+1) + "번째: " + str(client.guilds[i]) + "(" + str(client.guilds[i].id) + "):"+ str(client.guilds[i].member_count) +"명\n"
        total += client.guilds[i].member_count
...
async def on_guild_join(guild):
    ...
    if not server_number == None:
        log_system(guild.name + '에 가입이 확인되었습니다. 서버번호: ' + str(server_number) + '번, 서버멤버' + str(guild.member_count) + '명')
    return
